...
void main()
{
	...
	while (intContinue == 1)
	{
		...
		if (intContinue == 1) {
			fprintf(pfOutput, "\n");//newline only if there is more data 
									//this was necessary when part 2 was static
									//part 2 still relies on this
		}
	}
	fclose(pfOutput);
...
void GetInputs(udtSurveyType* audtSurveyList, int intArrSize) {
	...
	char* strDate;
	char* strState = (char*) malloc(8 * sizeof(char));
	char* strCounty =(char*) malloc(20 * sizeof(char));
	char* strRace = (char*) malloc(20 * sizeof(char));
	strState = "";
	strCounty = "";
	strRace = "";
	...
	StringCopy(audtSurveyList->strDate,strDate);
	free(strDate);//this doesn't break

	StringCopy(audtSurveyList->strState, strState);
	//free(strState);  //I know I should do this here, but it breaks, so I didnt

	StringCopy(audtSurveyList->strCounty, strCounty);
	//free(strCounty); //I know I should do this here, but it breaks, so I didnt

	StringCopy(audtSurveyList->strRace, strRace);
	//free(strRace); //I know I should do this here, but it breaks, so I didnt

	audtSurveyList->intHouseholdMembers= intHouseholdMembers;
	audtSurveyList->fltIncome = fltIncome;
...
char* FormatDate(int intMonth, int intDay, int intYear) {
	...
	if (intDay < 10) {
		sprintf(strDay, "0%d", intDay);//pad with zero if necessary
	}
	else {
